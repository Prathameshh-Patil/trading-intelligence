...
**The 7-digit CVD mode is implemented and typechecked but was not photographed** — four capture
attempts failed on display sleep and the window sitting on a non-visible Space, and it was not worth
a fifth. The field it lands in carries `tabular-nums`, `nowrap` and `text-overflow: ellipsis`, so it
is guarded by construction, but *guarded by construction is not the same as seen* and this row is
open.
...
- **Nothing was clicked.** Accessibility is still ungranted, so the failure modes were fired
  programmatically on mount and the five buttons are unproven. The modes work; the *controls* are
  untested. Same gap as W1D1's navigation, same one permission closes both.
- **The 7-digit CVD screenshot**, above.

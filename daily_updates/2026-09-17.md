...
## 12. Late — `mathematical.md` supersedes the programme, and three days are planned around it

> §11 called the day ended and it was right about the shell lane. **This is the signal lane, and it
> reopened after it.** Worth reading against §11's closing list: one of the things it says *"does
> not exist on any machine here"* turns out to be half-solved — see the `EA_DOMProbe.mq5` note below.

A new spec landed untracked in the repo root: `mathematical.md`, a mathematically explicit
GC/XAUUSD magnitude-expansion framework — Yang-Zhang + GARCH ensemble, volume-clock OFI, Hawkes
arrivals, Hurst/FBM, a three-state HMM, a calibrated classifier, a sweep/reclaim limit entry, and a
prop-firm risk overlay. **Six decisions were taken on it tonight and they change the track:**

| | Decision |
| :--- | :--- |
| 1 | `mathematical.md` **supersedes** the Track A/B research programme |
| 2 | Execution instrument is **spot XAUUSD on a CFD prop firm**, not GC |
| 3 | Trades-only order-flow proxies are **dropped**, not built — there is no tape to proxy |
| 4 | The 18/21/25 Sep scheduled items are **suspended for three days**, recorded as missed per §7's rule |
| 5 | The block is **Fri 18 – Sun 20 Sep** |
| 6 | Day 1 verifies a five-year bar source before anything is built on it |

Written up as [`docs/superpowers/specs/2026-09-18-magnitude-expansion-design.md`](../docs/superpowers/specs/2026-09-18-magnitude-expansion-design.md)
and [`docs/superpowers/plans/2026-09-18-magnitude-expansion.md`](../docs/superpowers/plans/2026-09-18-magnitude-expansion.md).

### Two things measured tonight that contradict what this repo currently records

⚠️ **`analysis/SPOT_FEED_CHECK.md`'s verdict is wrong about the source.** It concludes *"Dukascopy
is finished as a bulk source from this address."* Measured tonight, ~23:20–23:40 IST:

```
hourly tick endpoint, 3 cold probes after 3 days' rest    2 ok / 3     then a wall
budget probe, 6 consecutive                               0 ok / 6
re-probe, retries=3 timeout=20                            0 ok / 2     incl. an hour that worked 10 min earlier
daily candle endpoint, 12 consecutive                     0 ok / 12    HTTP 429, explicit
```

**Two corrections fall out.** First, the budget from a three-day rest is **~2 requests, not the ~17
the 14 Sep re-measurement recorded** — that figure is optimistic by an order of magnitude. Second,
and the one that matters: `dukascopy.py` surfaces resets and timeouts, so the wall has been read as
an IP ban for a week. **It is an explicit HTTP 429**, and its own JSON body names the sanctioned
path — Dukascopy's data-export docs, which document an **S3 requester-pays bucket**
(`cfg-public-proper-wallaby`, `eu-west-1`, daily `.bi5` files; their worked example is EUR/USD's
full history at 2.6 GB / 26,586 files / ≈ $0.06). **The transport conclusion stands; the source
conclusion does not**, and route 2 has been gated on the wrong one since 11 Sep. E0 verifies the
bucket on Friday before anything is built on it.

🔁 **A prior-art sweep of `vrd07/Quant_Trading`** — Varad's own repo, 158 commits, same instrument
— found three things that transfer. `scripts/fetch_dukascopy.py` pulls **daily candle files**
(`BID_candles_min_1.bi5`), ~1,300 requests for five years against 43,800 hour-files; that is the E0
fallback. `mt5_bridge/EA_DOMProbe.mq5` is a read-only `MarketBookGet` probe with an explicit
*"broker likely publishes no book"* path — **it is R7's depth check, already written.** And
`src/backtest/walk_forward.py`'s deploy gate (OOS Sharpe ≥ 0.5×IS, PF ≥ 1.0, WR within 10pp)
retired six strategies there and is adopted here.

⚠️ **One bug recorded so it is not lost:** that repo's `scripts/download_historical_data.py` maps
`XAUUSD → GC=F` and calls the two *"within ~$5"*. `SPOT_FEED_CHECK.md` §3's 2025-06-18 cross-check
measured the basis at **≈ +19**. That file labels futures as spot.

### A correction to tonight's own arithmetic, recorded rather than quietly fixed

An intermediate cost analysis put a $0.30 XAUUSD spread at **8.8 bp** and concluded the spec's
claimed 42% win rate was below break-even at a prop-firm venue. **That was a factor-of-ten error**
— $0.30 on $3,400 is **0.88 bp** — and the conclusion is withdrawn. Corrected, the three venues are
GC **0.29 bp**, prop firm **0.88 bp**, Dukascopy **1.91 bp**, and EV is positive at 42% at all
three. **The spread is not what kills this strategy.** What does bind is §13's own slippage
exclusion: at `D = $2.00/oz` the Dukascopy-width spread alone is 32% of risk against a 25% ceiling,
so **the bottom ~20% of §10's stated `$2.00–$5.00` stop range is unreachable under the spec's own
rule.** §10's range is restated as `$2.60 ≤ D ≤ $5.00` and E3 measures the real floor on Friday.

**Nothing was run against the strategy tonight.** No feature code, no backtest, no result — the
three days start Friday with the seam freeze and the pre-commitments, in that order.

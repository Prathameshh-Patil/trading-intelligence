 ...
 *
 * TEMPORARY SHIM (Tauri port): the extension build talks to
 * chrome.storage.local. There is no chrome API inside a Tauri webview, so this
 * copy is backed by window.localStorage only and seeds the same demo data.
 * The real desktop version will persist to the filesystem or a local SQLite
 * file via a Tauri command — that is a later task. Swapping the backend means
...
/**
 * TEMPORARY SHIM (Tauri port): kept so SidePanel.tsx and capture.ts can stay
 * byte-for-byte identical to the extension. Inside a Tauri webview there is no
 * chrome runtime, so this is always false.
 */
...
async function writeRaw<T>(key: string, value: T): Promise<void> {
  try {
    ...
    // Storage full or blocked. Non-fatal for a preview.
  }
}

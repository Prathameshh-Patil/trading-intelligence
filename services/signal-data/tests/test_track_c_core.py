...
def test_the_hurst_window_reaches_every_scale_the_ladder_declares() -> None:
    """**The window is derived from `SCALES`, not chosen — and that is the
    whole argument for the number.**

    A scale survives only if the window holds four of it, measured against
    `window - 1` because DFA differences its input and has one point fewer. So
    the smallest window that reaches the whole declared ladder is
    `4 * max(SCALES) + 1 = 1025`. **Any smaller window silently truncates it**,
    and a truncated ladder is not a cheaper estimate of the same quantity: H is
    scale-dependent, so it is a different quantity under the same name.

    HISTORY, because the failure it fixes was invisible. Until 2026-09-19 this
    was 256, which reached only `(4, 8, 16, 32)` -- four of seven rungs. That
    is exactly the ladder `test_dfa_is_biased_high_on_a_short_scale_ladder_and_
    agreement_fails_there` was written about on 18 Sep, where DFA reads 0.563
    against a **true** 0.5. The bias was documented in a test and then
    configured into production, and it surfaced as `hurst_agree` refusing three
    bars in four. On 300 random walks at window 256 the estimators read 0.562
    and 0.420 -- a +0.142 gap against a 0.05 tolerance, agreeing 16.7% of the
    time, while the real archive agreed 26.2%. **A gate that a random walk fails
    more often than the market does is not measuring the market.**

    This test is deliberately written against the RULE rather than against 1025,
    so it keeps holding if `SCALES` ever changes -- and fails if the window is
    lowered back under it.
    """
    cfg = config.load()
    window = config.i(cfg, "regime.hurst_window_bars")

    # The rule, not the number: four windows of the largest scale, plus the
    # point DFA loses to differencing.
    assert window >= 4 * max(hurst.SCALES) + 1, "the window truncates the declared ladder"
    assert tuple(regime.hurst_scales(window)) == hurst.SCALES

    # And it is the smallest such window -- the ladder is the reason for the
    # cost, so paying more of it than the ladder asks would need its own reason.
    assert window == 4 * max(hurst.SCALES) + 1 == 1025
    assert tuple(regime.hurst_scales(window - 1)) != hurst.SCALES

...
| :--- | :--- | :--- | :--- |
| **0** | Aug 30 | Days 1–3 debt is cleared; every open checkbox in `current.md` is closed or explicitly killed | A real Tauri window screenshotted on-device · ~~real Databento row counts~~ **met by `8aece67`** · extension reloaded in a real Chrome profile · one live Claude analysis returned · the S1 fixture committed · a reference-chart path chosen · **the Anthropic key rotated** |

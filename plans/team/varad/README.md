...
| Day | Work |
| :--- | :--- |
| **Wed 26** | ⚠️ **First: rotate the Anthropic key.** `DELTA_CVD_FINDINGS.md` §5 — a real key sat in the tracked `.env.example` (uncommitted, absent from branch history, one `git add -A` from being pushed) **and was pasted into a chat transcript.** Repo is clean; the key should not still be live. Ten minutes, before you put money on the account. Then: **settle the analysis-backend decision and run the estimate.** The artifact budgets *"LLM extraction — vision calls for chart context, ~10 users — $20–50/mo"*, which is a hosted API. Under bring-your-own-feed the model never sees market data — only a screenshot's symbol and timeframe. Ollama was the privacy answer to a privacy problem the architecture has now deleted, and it can't back a deployed API anyway. **Recommendation: add $5 Anthropic credit and stop deferring.** ~3,000 analyses, zero code change; everything else in PR #3 is already verified. Then `pull_futures_trades.py --estimate-only`, one month GC + NQ. *(closes #1's decision half, #4's secrets half)* |
| **Thu 27** | ~~Run the Databento pull~~ — **done 24 Aug by Prathamesh (`8aece67`), and done well.** 1,616,772 GC trades, July 2026, $2.52. Aggressor convention confirmed three independent ways. **Read [`DELTA_CVD_FINDINGS.md`](../../../services/signal-data/analysis/DELTA_CVD_FINDINGS.md) end to end before you touch anything** — it will save you a day. Then: **(1)** cut `data/fixtures/gc_ticks_1session.parquet` from `data/gc_trades.parquet` — the 2026-07-16 GCQ6 session, 77,532 trades; `data/` is gitignored so it needs an explicit exception. **That's S1.** **(2)** Own the reference-validation blocker with Shreyas — see [`../week-00.md`](../week-00.md). **(3)** Decide NQ: never pulled, ~$11.42 unspent, and Friday of Week 1 picks one instrument on evidence you may not have. |
| **Fri 28** | **Freeze S3.** Write `services/api/tests/fixtures/keys_fake.py` — 30 lines, all six branches of key-validate, port 8001. Not needed until Week 5. **Write it now anyway, while there is slack, because Week 5 has none.** Commit S1 and S3 in `contracts.md` as the frozen record. |

**Float:** Thursday, if the pull finishes early — run the first live Claude analysis
(`LIVE_API_TESTS=1 uv run pytest`, 5 tests). Last unverified thing in PR #3, ten minutes once there's
credit.
...
| **Wed 02** | *(Pulled forward to Monday — use this day for the second half.)* Outlier detection from `strategy.md`: ≥60 on NQ, ≥200 on ES, **scaled appropriately for GC — and write down *how* you scaled it**, because that number gets argued about in Week 4. Output for one session: timestamped outlier prints with price levels and cluster boundaries. Still a notebook. |
...
| **Fri 04** | Present the distribution against **Thursday morning's committed threshold** — not the best-looking cut of it, the one you committed to. Shreyas asks the uncomfortable questions; that's his job. Then: launch instrument (GC or NQ, **one**), feed vendor, and the decision record naming **what would change your mind**. |
...
| Wk | You must have |
| :--- | :--- |
| 0 | ~~Real Databento row counts and aggressor split~~ **already met by `8aece67`** · `gc_ticks_1session.parquet` committed · `keys_fake.py` committed · **Anthropic key rotated** · a reference-chart path chosen |

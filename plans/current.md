...
- [ ] **The two S7 seam mismatches are unchanged and still need the room.** `PipForecast.bucket`
      carries `regime` — `regimes.py`, parked, does not port to spot — and has no field for `phase`,
      `vol_state` or `instrument`; and `reach` is `{target, stop, p}[]`, which cannot carry
      `p_target_max`. Flagged before the build, untouched by it.
- [x] **`first_touch` rewritten the same night — 116 minutes to 3.4, a 34x cut.** ~188 µs per leg
      iteration was pandas-per-row overhead, and the giveaway was that a 15-minute window cost the
      same as a 60-minute one: the arithmetic was free and the slicing was all of it. The window
      depends only on `(t, side, horizon)`, so `excursions` now builds it three times per cell
      instead of 144 and `first_touch_from` answers each bracket in numpy. **`first_touch` keeps its
      signature and its meaning** and is the single-bracket spelling of the same code, so the repo

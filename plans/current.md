...
- [ ] **`first_touch` is 116 minutes of the 116, and it is fixable.** ~188 µs per leg iteration is
      pandas-per-row overhead: the window depends only on `(t, side, horizon)` but is rebuilt 48
      times per cell, and `session_ends` is recomputed on all 8,208 calls per month-side.
      Deliberately not fixed in this commit — it is shared machinery, and `reach_table.csv` as
      committed is the byte-for-byte reference any faster version has to reproduce.

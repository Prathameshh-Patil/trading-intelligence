...
| R5 | ⏱️ **Clock lane — step 3 built, run pending; three things still open** | Prathamesh, 6 Sep | [`plans/team/prathamesh/clock-lane.md`](team/prathamesh/clock-lane.md). `session_phase`, `event_proximity`, `anchors`, `m3_session_event`, `calendars.py` and `m3_profile.py` are built; **199 tests green**, `ruff`/`mypy` clean. Costs **4.6s/month, ~1.4 min for all 19** — M3 is cheap and re-runnable, unlike M1's ~32M-iteration sweep, so the performance risk R4 names is Varad's alone. **Open, in order:** **(a) ⚠️ Varad has not signed the phase boundaries** — they are frozen in **ET wall clock, not UTC**, against both §7's wording and the argument written into the empty block. The case is in `strategy-precommit.md` §4 and it is one sentence: a frozen UTC boundary is stable in its *number* and unstable in its *referent*, so `London-NY` would hold the NY handoff for seven months of the archive and mid-London for the other five. **(b) ⏳ The archive restore is in flight** — it was gone from this machine and the iCloud backup it documents does not exist; the re-pull is proven byte-identical against the manifest, and **the M3 run waits on it and on nothing else** (one month cannot be split). **(c) ⛔ `replay.py` (step 5) not started** — §5 requires a time budget written into that file before the first line of code and the blank is still blank; step 6 waits on a spot vendor. **Handed to Varad, not fixed here:** `s1.SESSION_SHIFT`'s +2h rolls the session date at 17:00 ET in winter, an hour inside `NY-Asia`, so for five months of the archive `session` and `phase` disagree about which day a bar is. `reach.py` keys on `phase`, so the rejoin is unaffected |

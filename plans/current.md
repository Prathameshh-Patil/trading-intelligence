...
### 2026-09-06 (later still) — the clock lane, and the archive restored · [`plans/team/prathamesh/clock-lane.md`](team/prathamesh/clock-lane.md)

**Step 3's code is built and green.** `session_phase`, `event_proximity`, `anchors` in
`features/portable.py`; `m3_session_event` in `strategies.py`; `calendars.py` +
`reference/us_releases.csv`; `m3_profile.py`; 33 tests in `tests/test_clock.py`. **199 green**,
`ruff` and `mypy` clean over 40 files.

- [x] **Prathamesh's §7 pre-commitments are filled — four, not three.** ±15 minutes either side of a
      release · the 2025-01…09 / 2025-10…2026-07 split · a 30-minute opening range · and the six
      phase boundaries. The fourth is new: `anchors` computes an opening range and a range has a
      length, so it is a threshold like any other. All in
      [`team/strategy-precommit.md`](team/strategy-precommit.md) §4–§7, in a commit, before the run.
- [ ] **⚠️ The phase boundaries are frozen in ET wall clock, NOT in UTC — Varad has not signed
      this.** It contradicts both `strategy-split.md` §7's wording and the argument written into the
      empty block itself (*"a UTC boundary is stable"*). **A frozen UTC boundary is stable in its
      number and unstable in its referent:** 12:00 UTC is 08:00 in New York for seven months of the
      archive and 07:00 for the other five, so `London-NY` would hold the NY handoff in summer and
      mid-London in winter — one label, two populations, §4.6's error through the clock. Marked
      "needs Varad's eye, not his nod" rather than ticked.
- [x] **🔴 The release calendar is transcribed from BLS and the Fed, never generated — and the
      archive is why.** "First Friday, 08:30 ET" is wrong on this exact data in both directions:
      **there is no October 2025 Employment Situation at all**, September's landed **2025-11-20**,
      and September CPI landed **2025-10-24**. A generated calendar marks a quiet Friday as payrolls
      *and* leaves the highest-volatility gold bar of that quarter in the null. Three tests pin it;
      `require_coverage` catches the other half, since an uncovered month is all-False and
      byte-identical to a genuinely quiet one.
- [x] **🔴 The 19-month archive was gone from this machine and has been restored.** Only 2026-07 was
      on disk — 1.6M rows of 46M — and **the iCloud backup `gc_data_manifest.md` documents does not
      exist**, so that line in the manifest described a state of the world that had stopped being
      true. Priced the re-pull for free at **$66.72** (net **$64.20**; 2026-07 re-derives from its
      cached DBN). **Pulled 2025-01 alone first, $2.54, to test the assumption nobody had tested —
      it returns byte-identical**: same `sha256`, same 13,483,050 bytes, same 1,600,148 rows, same
      `GCG5·GCJ5`. **The pipeline is deterministic and the parquets are re-derivable, so the
      manifest was the real backup all along.** Restore of the remaining months in flight; each
      verified against the manifest before it counts.
- [x] **A bug caught and pinned, and it is not confined to this lane's subject.**
      `DatetimeIndex.asi8` returns the index's **own** resolution — ns off `read_parquet`, µs off
      `pd.Timestamp`. Compared against a nanosecond window it divides every gap by a thousand, so a
      bar four hours from a release read as fourteen seconds from it and **the event arm fired on
      every bar**, while still returning a well-formed boolean Series of the right length and dtype.
      `_epoch_ns` pins the unit. **Audited: no other instance in the repo.**
- [x] **`m3_profile.py` costs 4.6s/month — ~1.4 min for all 19.** Varad's "measure one month before
      launching 19" applied to this lane too, and the answer is that **M3 is cheap and can be re-run
      freely**; it is ~110k `first_touch` iterations against the sweep's ~32M. The performance risk
      is M1's alone.
- [x] **`m3_profile.py` imports `ATR_BP_EDGES` rather than defaulting.** Written this afternoon
      refusing to default at all, because the edges did not exist; Varad's commit landed them
      mid-session. **Reading his committed constant is not the same act as picking edges** —
      `--atr-edges` survives only as an override that prints `SENSITIVITY CHECK … NOT the reported
      profile`.
- [ ] **⛔ The M3 run waits on the restore finishing, and on nothing else.** §6's split needs all 19
      months and one month cannot be split — M3 on July alone is not a weak result, it is no result.
- [ ] **Step 5 `replay.py` not started, deliberately.** `strategy-split.md` §5 requires a time budget
      written into that file before the first line of code, and the blank is still blank. Step 6
      waits on a spot vendor; `mid` / `spread_bp` / `quote_rate_z` are still signatures.
- [ ] **A finding handed to Varad, not fixed here — `s1.py` is shared spine (§2).**
      `SESSION_SHIFT`'s +2h rolls the session date at 22:00 UTC: **18:00 ET in summer, exactly the
      Asia open, but 17:00 ET in winter, an hour inside `NY-Asia`.** For five months of the archive a
      bar's `session` and its `phase` disagree about which day it belongs to. `reach.py` keys on
      `phase`, so the rejoin is unaffected; it closes with the calendar S8 drafted.
- [x] **The split held its first real test.** Both lanes pushed to `main` within minutes. Three
      conflicts, all in files both lanes were expected to touch — the constants block in
...
### 2026-09-10 — stage 7 built and served: the reach table, and the two axes are not independent · [`analysis/REACH.md`](../services/signal-data/analysis/REACH.md)

**Step 7 of `ARCHITECTURE.md` §6, and the rejoin `strategy-split.md` §6 has been pointing at since
6 Sep.** `reach.py` ran on all 19 months and both sides — 207,984 legs, 116 minutes — and
`analysis/reach_table.csv` is committed: 10,368 rows over 144 cells, the key
`(instrument × atr_bp × phase × vol_state × side)` exactly as `strategy-precommit.md` §10 froze it
the day before the file existed.

- [x] **116 of 144 cells clear the 400-leg floor — 81%**, and 8,256 of 10,368 rows are served.
      The thinnest cell that clears holds 428 legs; the fattest that does not holds 375.
- [x] **The §10 prediction is scored and it is wrong, in the optimistic direction.** It expected a
      third to a half of the cells to clear and the table to be "honest and mostly empty at the
      edges". The table is mostly full. Both misses come from the same mistake — predicting density
      from leg count alone, when what actually kills a cell is the two axes interacting.
- [x] **The finding: `phase` and `vol_state` deplete each other where they cross.** Both transition
      phases lose their entire CONTRACTING state, 8 of 8 each, 16 of the 28 dead cells. Measured
      rather than inferred: CONTRACTING is **4.7%** of `London-NY`'s legs and 10.0% of
      `Asia-London`'s, against 28–36% in the three phases that are not handoffs. A 1–1.5 hour
      handoff window containing the 08:30 release is a place where volatility is rising by
      construction. **`London-NY` carries 50% more legs than `Asia-London` and loses more cells**,
      which is how we know size is not the mechanism.
- [x] **That is `atr_bp`'s quiet-hour artefact found for the third time** — `M1_SURFACE.md` §3 saw
      it as 73% of passing cells off 17% of the surface, `M3_CLOCK.md` §2 saw the reach lift fail to
      convert, and this is the same fact from the volatility axis instead of the clock.
- [x] **The tie band reproduces M1 independently** — 8.2% of served rows undecided against M1's
      10.2%, on a different cell partition from a separate build. Band median 0.0019, max 0.107.
- [x] **No edge, and none was owed.** `ev_sym` positive on 4.4% of served rows, median −0.0421 ATR
      — the cost floor. The pass line fires on 4.0% against M1's 4.6% already called noise. Stage 7
      tabulates; it does not select.
- [x] **Written up for Prathamesh in plain language with runnable examples** —
      [`prathamesh/reach-table-explained.md`](team/prathamesh/reach-table-explained.md). His `phase`
      column is half the key and the finding is about his axis, so it is his to read first. Every
      code block in it was executed before it was committed.
- [ ] **The thresholds are frozen from here and the dead cells stay dead.** The surface has now been
      looked at, so moving a phase boundary or the `rv_slope` cut would be a fit rather than a fix —
      `strategy-precommit.md`'s standing rule, and it binds Varad the same as Prathamesh.
- [x] **The two S7 seam mismatches are closed — S7 frozen 3 of 3, the same evening the table
      shipped.** Prathamesh proposed the amendment from the side that has to render the numbers;
      Varad approved directly to him and Shreyas approved via Varad. **Both verbal, neither a logged
      artefact from the moment** — the basis rows #2 and #8 are closed on, recorded that way in the
      spec's amendment block rather than dressed up as a paper trail. `bucket` now carries
      `instrument`/`atrBucket`/`phase`/`volState`/`side` instead of `regime`, and every probability
      carries `pMax` beside `p`. **`contracts.md` now registers S7 as its seventh seam** — it had
      never carried it, so the freeze register did not know the contract existed. The type is
      pointed at, not copied: a second hundred-line copy that must agree with `forecast.ts` is the
      `reach_table.json` problem again, and that one needed a test to catch.
- [ ] **S7 frozen does not mean S7 is real.** The type, the fake and one consumer are in
      `apps/desktop`; there is no real implementation and no `services/api` route. Writing those
      fills slots the contract specifies and is not a contract change. Changing the *shape* after
      they exist is a four-file commit under the freeze discipline.
...
### 2026-09-11 — step 5 ran: the nearer barrier is touched first, and one number was forced · [`analysis/REPLAY.md`](../services/signal-data/analysis/REPLAY.md)

**The largest work item in Track B, and the one `strategy-precommit.md` says M4 is blocked on.**
`replay.py`, standalone, 19 months, 127s. It measures and does not fix: `backtest.first_touch` is
untouched, per `strategy-split.md` §5.

- [x] **The budget was written into `strategy-split.md` §5 before the first line of code** — 3 days,
      which is what ARCHITECTURE §9 open question 5 asked for and which closes it. One day on the
      tape walk proven against a hand-checked leg, one on the archive, one on the write-up.
- [x] **89,646 ties over 14,736,048 leg-brackets — 0.61%, all of them ordered by the tape.** The
      rule sends every one to the stop; the tape sends 37,301, or 41.6%, to the target.
- [x] **It reconciles with `reach_table.csv` to the unit — 89,646 against 89,646** — by a different
      path entirely, and **the expected number was computed and written down while the run was still
      on month 12**, which is the only version of that check worth anything.
- [x] **The finding: `P(target first)` is monotone in the stop/target ratio**, 0.178 at 0.125
      through exactly 0.500 at symmetry to 0.649 at 2.0. The nearer barrier is touched first, which
      is the sensible null and is evidence the walk is sound rather than a surprise. **The
      consequence is about the rule:** `ties="stop"` is a blanket answer to a per-bracket question —
      roughly right where the stop is nearer, and wrong more often than right where the target is,
      which is 10 of 72 brackets and 12.9% of the ties.
- [x] ⚠️ **One number was forced and looked like a result.** At `target == stop` the pooled split is
      exactly `0.500000` on 17,592 ties, 19.6% of the archive's — a long and a short on the same bar
      with equidistant barriers are the same two price levels, so pooling the sides cannot produce
      anything else. Nearly published as "ties are a coin flip". Caught by asking why a number was
      *exactly* round. Per side the headline survives: short 42.4%, long 40.8%.
- [x] **The tests passed first try and three were wrong**, found by mutating the module rather than
      trusting the green — the tie-population test could not tell `==` from `<=`, the missing-barrier
      test passed by accident on a `-1` index, and `& ex.valid` could be deleted with nothing
      failing. All ten mutations now fail. **Every number in `REPLAY.md` was re-derived from the CSV
      before it was committed** — 52 assertions.
- [ ] **No `p_target` moves and none is proposed.** That is a change to `backtest.first_touch`, a
      shared-spine change with Track A's tests as the gate, and it is Varad's — and only if this
      number earns it.
- [ ] **M4b is still open, and is now cheap.** The Path-Dependent Exit needs the intrabar
      MFE-vs-retracement ordering; the tape walk that answers it is written and the extra pass is
      small. Left out deliberately rather than absorbed into a 3-day budget — it wants its own item.

Full detail: [`analysis/REPLAY.md`](../services/signal-data/analysis/REPLAY.md) and
[`daily_updates/2026-09-11.md`](../daily_updates/2026-09-11.md).

---

### 2026-09-12 — M4b answered: bars carry the sequence, so the PDE's blocker was never there · [`analysis/M4_PATH.md`](../services/signal-data/analysis/M4_PATH.md)

**Prediction and kill condition committed in `d648938` before the code that reads them.**

- [x] **13,744 of 327,135 legs have MFE and MAE in the same bar — 4.20%.** The other 95.8% are
      ordered by the bars themselves, so §11's kill condition fires: the tick replay this candidate
      was blocked on was never the blocker.
- [x] ⚠️ **The prediction is scored and the 15m horizon breaches it — 10.15% against "under 10%".**
      Recorded as a miss, not rounded away. **The reasoning was right and the arithmetic was the
      miss:** §11 said the extremes should land in different bars "simply because there are several",
      then wrote "3, 6 or 12 bars" and reasoned as though the number were always large. A 15-minute
      horizon is **three** bars — and 15m is exactly where a PDE rule would live, since S4's own
      conditions are `mae > 0.6 * stop` in the first 5 bars and `speed_to_mfe < 5 bars`.
- [x] **There is no conditioning structure.** 63 non-thin cells at 15m span 7.13%–12.89%, **1.8×**,
      against the 276× the tie rate spans over 72 brackets on the same archive. The variation is
      real — χ² 186.7 on 62 dof, 3.01 per cell — and too small to act on. **No pocket exists where
      bars are blind enough to justify a restricted rule.**
- [x] **Ordering, not magnitude, and that was fixed before the run.** Bar MFE and tick MFE are the
      same number because a bar's high IS the maximum tick price inside it — verified at `0.00e+00`
      over 18,345 legs by independent code paths.
- [x] ⚠️ **The long/short mirror forced a number for the second time in two days.** `side` is a
      relabelling: the same two prices, swapped roles. Both summary functions drop it rather than
      summing, each with a test. Worth treating as a property of these measurements.
- [ ] **M4 is still not decided.** §415 orders it after tick replay **and** route-2 spot; spot is
      blocked on the Dukascopy IP block. This removes one blocker and supplies evidence for the call
      without making it.
- [ ] **No PDE rule is implemented and none is proposed** — §11 is explicit that S4's five
      thresholds are invented constants.
...
| B | Make `apps/desktop` behave like a real overlay — transparent, borderless, always-on-top, click-through toggle. Prove it floats over a live MT5 demo and that click-through reaches MT5 underneath | **Prathamesh**, W1D2–W2 | No longer gated on A1/A2 — it is Week 1 Day 2 and Week 2 in [`team/phase-1-kill-week.md`](team/phase-1-kill-week.md). Install the MT5 demo terminal first if nobody has it. **Click-through toggle built 5 Sep, `e0171bf`** — whole-window (Tauri's `setIgnoreCursorEvents` has no per-pixel `forward` option, so per-region hit-testing is off the table), armed from the toolbar. **On-device macOS bug found and fixed the same day:** the original `Escape`-to-disarm path relied on the `WKWebView` holding keyboard focus, but Cmd+Tab / the Dock reliably reactivate the *app* without reliably returning focus to that specific webview — the window stuck armed and unclickable with no way back short of killing the process. Fixed with an OS-level global hotkey (`Cmd/Ctrl+Shift+K` via `tauri-plugin-global-shortcut`, `apps/desktop/src-tauri/src/lib.rs`) that disarms directly from Rust regardless of focus. **All of this is macOS-only so far — Windows and Linux are untested**, not confirmed fixed: WebView2 (Windows) may not share `WKWebView`'s focus-loss behaviour, WebKitGTK (Linux) is the same WebKit family and should be assumed to share it until tested, `setIgnoreCursorEvents`'s all-or-nothing behaviour is unconfirmed on either backend, and global-shortcut registration is a known weak spot under Wayland. No owner or date yet — there is no Windows or Linux dev machine in the loop. Full writeup in [`daily_updates/2026-09-05.md`](../daily_updates/2026-09-05.md#the-click-through-toggle-and-the-macos-focus-bug-that-broke-its-first-design). **W2D2's summon/dismiss hotkey built 17 Sep** — `Cmd/Ctrl+Shift+O`, same OS-level registration as the disarm, toggles from the window's own visibility on the Rust side and takes focus on summon. **Verified from outside the app on macOS:** with Chrome frontmost, `⌘⇧O` sent via System Events hid then showed the window, four toggles logged, `⌘⇧K` unaffected. Handler identity is now by registration (`on_shortcut`), which is what made a second shortcut possible without the modifier-matching bug the first disarm had. **W3D1's status indicator pulled forward, same day** — the feed connection moved out of `FlowView` into the shell (`lib/feed.ts`, one connection, every view reads it) and a four-state `FeedPill` sits in the topbar: hue + glyph shape + label per state so reduced-motion cannot collapse two, `stale` carries a time-in-state counter, live is a wordless dot because 380px does not hold the word. Two bugs found by running it: `connecting`/`stale` shared a tone, and the first counter read `Stale 5512993s` off the fixture's July timestamps. **Credential entry to the keychain not built** — `FeedCreds` stays open until #9 supplies the fields. **W3D2 done the same evening** — `lib/api.ts` deleted; the Analyze view reads `lib/context.ts` (S6, screenshot → `CaptureContext`, mock behind `VITE_CONTEXT`) and then the engine, and no signal number crosses a network from the desktop any more. W3D4's honest states (no feed for this chart, unsure symbol, nothing readable, feed stale) are built and each shows no number. `services/api` still has no extraction route — the seam is ready, the route is not. **W3D1's credentials built too, correcting the note above** — storage was never blocked, only the field names were: `src-tauri/src/creds.rs` puts one entry per vendor in the OS keychain (`keyring 3.6.3`, native backends only, round-trip tested against the real macOS keychain), a generic `field = value` card in Order Flow saves and reconnects, values never return to the DOM after save. The Ironbeam field names still arrive with #9. Still open in this row: the compositing measurement (W2D5) and the click-through proof over a real MT5 *desktop* terminal — none is installed on any machine in the loop. See [`daily_updates/2026-09-17.md`](../daily_updates/2026-09-17.md) §5, §8–§10 |
...
| R1 | 📐 **Four L1-only strategies — analysis written, none built** · **reconciled 6 Sep against a second independent proposal, see R2** | Varad | Written 5 Sep at Varad's request: [`docs/strategy/four-l1-strategies.md`](../docs/strategy/four-l1-strategies.md), reading [`docs/strategy/xauusd-regimes.md`](../docs/strategy/xauusd-regimes.md) against what this repo has already measured. **The finding that drives it:** three of the source document's four strategies need volume (VWAP, `volume > 150% average`), and spot gold has none — the same fact that cut A2 — so as written the set is half unportable, and its two portable members are the two `horizon.py` measured as a random walk. **The proposed replacement four are S1 bracket-geometry sweep, S2 compression→expansion, S3 anchor-distance reversion, S4 clock/event conditioning; only S3 is directional**, because §1.3's arms landed on their own null at a power that would have seen a 30% relative lift, and `PipForecast` sells a distribution rather than a side. **Three architectural consequences, each a real blocker rather than a preference:** `s1.TICK = 0.10` is hardcoded and every tick-denominated number in the repo flows through it, so volatility buckets must be redefined in **basis points** before any GC number can be read as a prior for spot; `regimes.py` clusters on `cvd_slope` and `cvd_persistence`, so **the committed regime labelling does not port to spot at all** and needs a refit on portable features; and quote rate — the only activity measure available on both — is vendor-relative (AllTick 1 Hz / ~2% delivery vs Databento every update) so it enters as a trailing z-score, never a raw threshold. **Steps 2–4 of §7's order need no new data, no vendor decision and no feed** — they run on the 46M trades already in `data/`, which is deliberate: if S1, S2 and S4 all come back flat, that is worth knowing before the spot-vendor question is answered. **Nothing is built and no threshold is committed**; §10 lists five decisions that are Varad's, the first being whether S1's geometry grid gets committed before the sweep |
| R2 | 🔀 **Two L1-only proposals reconciled — and there is no quote data in this repo** | Varad, 6 Sep | `strategy-architecture.md` (`e4a10f3`) landed hours after R1's `four-l1-strategies.md` (`a12d87d`), independently, against the same constraint. Reconciliation: [`docs/strategy/strategy-reconciliation.md`](../docs/strategy/strategy-reconciliation.md). **They agree on all ten structural conclusions** — same portable surface, same bp normalisation, same `instruments.py`/`has_flow` and `features/portable.py`, same magnitude-over-direction, same MDE discipline. Two independent passes landing on the same architecture is the strongest evidence available that it is forced rather than chosen; §2 records it as settled. **The finding neither document checked, and it decides two of six slots: every one of the 19 months is the `trades` schema.** `find data -name '*.dbn.zst'` returns 19 trades and 19 definition, nothing else; the one TBBO pull that ever ran did not keep its raw cache, and the surviving `gc_footprint_quoterule_2026-07-15.csv` is aggregated to price levels with no bid or ask. **And the two spread/quote-rate features are not one purchase:** spread needs `tbbo` or `bbo-1s`, but **quote rate is a count of book updates so it needs `mbp-1`** — TBBO observes the book only at trade times. `mbp-1` on GC is a different order of magnitude from 46M trade rows and **the cost estimate has never been run.** **Merged four: M1 geometry frontier · M2 vol momentum + compression · M3 session + event conditioning · M4 contested.** M1 runs first because the incoming spec's S2 hardcodes six brackets — (100,25,45m), (120,30,60m), (90,25,40m) and more — which are guesses at points on a surface nobody has measured, and it calls (70,20,30m) a "base case" when `BASE_RATES.md` measured it at −4.25 ticks in that bucket. **Adopted from the incoming spec:** its six-phase session taxonomy, its event table, its edge-case tables, `validate_l1_data`, the `StrategyReport` shape, and its "avoid negative EV rather than find alpha" framing, which is the better one. **Rejected:** `liquidity_score = (1/spread_z) * max(0, quote_rate_z)`, which has a pole at zero and flips sign mid-range; `SESSION_MULTIPLIER` and the `+400%/+350%` vol shifts, which are invented constants presented as pre-committed thresholds (this repo carries **no defaults at all** for exactly this reason — the fix is a change of type, a measured table with an N per cell, not a multiplier); and §9's predicted 0.22–0.25 `p_target`, which is the claim `FAMILIES.md` falsified the day before at −0.0058/−0.0003 against power that would have caught a 30% relative lift. ⚠️ **Its risk section specifies Kelly sizing and daily loss limits, which crosses the pipeline design §2.3 freeze** — *"position sizing is the user's, never ours … no field in §6's contract may become one"* — and that is the SEBI RA question pending Shreyas's CA call. **Its S4 (path-dependent exits) is a good idea blocked on resolution, not on a purchase:** the rules depend on the intrabar order of MFE and retracement, which an OHLC bar does not record — `regime_filter.py` already named this as *"exactly what D4 must measure at tick resolution"*, and that replay does not exist. **Its 14-day roadmap ends in "production-ready" with Day 12 as spot-feed integration**, when C1 reopened 5 Sep and no spot vendor exists; it is neither withdrawn nor in `plans/team/`. **Recommended order M1 → M3 → M2 → spread test → M4** — the first three need no new data, no feed and no purchase, so three kill conditions can fire on data already paid for before `mbp-1` is bought. **The one-session experiment that decides M4, and neither document asked it:** does GC top-of-book spread have enough variance to filter on? GC sits at one tick most of the time; if so `spread_z` is near-degenerate and the Adverse Selection Filter is a spot-only strategy wearing a cross-instrument label. Pull `tbbo` for 2026-07-16 only — already the validated reference session — and **keep the raw cache this time.** **Nothing is built and no threshold is committed.** ✅ **BUDGET DECIDED 6 Sep, 00:17 — the whole programme runs for $0 and the one-session `tbbo` purchase is off.** Five of six candidates run on the 46M trades already on disk; the only paid item in either document was GC quote data for the Adverse Selection Filter. **Correction on the Path-Dependent Exit:** it is blocked on *tooling*, not a purchase — trade-by-trade replay needs `timestamp, price, size`, which every month on disk carries, and it resolves the intrabar MFE-vs-retracement ordering an OHLC bar cannot record. Largest work item in the set, costs nothing but time. **Route 2 chosen for the spread question** over Roll (1984)'s serial-covariance estimator (undefined when the covariance is positive, common in trending tape) and trade-intensity-as-quote-rate (free and honest, not the same object): **get spot XAUUSD bid/ask, where quotes are the entire feed.** The argument is not the price — the problem inverts across instruments. On GC quotes cost money *and* the variable is probably degenerate (one tick the overwhelming majority of the time, making `spread_z` a z-score of a near-constant and `spread_z_max = 1.5` a threshold on noise); on spot, spread is free and actually moves. ⚠️ **Dukascopy reachability is unverified from this machine** — `DELTA_CVD_FINDINGS.md` §4 recorded it unreachable once, that line was rewritten for a different reason, and it was never retested. **Step one of route 2 is a ten-minute check that a download completes.** Fallback: MT5 demo (free live L1 + exportable tick history, doubles as the Day 2 platform check). **Caveat that survives either source:** spot spread is a broker pricing decision, not a market outcome, so the result is a yes/no on the hypothesis and **never a threshold that transfers** to GC or another broker. **New order, all free:** M1 → M3 → M2 → tick replay + PDE → route 2 → M4 decided. **The `mbp-1` question is deferred, not answered** — price GC quote data only if route 2 comes back positive; both cost-estimate flows return a number without spending anything |
| R3 | 🧭 **Two tracks declared — order flow parked, L1 strategy active** | Varad, 6 Sep | [`docs/strategy/ARCHITECTURE.md`](../docs/strategy/ARCHITECTURE.md), the strategy track's complete pipeline (sources → normalise → portable features → strategies → legs → null+power → reach table → calibration → product surface), plus a pointer at the top of [`docs/ARCHITECTURE.md`](../docs/ARCHITECTURE.md) so the two do not drift. **Track A** (delta/CVD/absorption/footprint, GC only, needs unpriced `mbp-1`) **is PARKED**; **Track B** (price/spread/quote-rate/time, GC **and** spot, **$0**) **is ACTIVE**. **"Parked" is defined precisely so it does not rot:** nothing deleted, moved or rewritten — `features/orderflow.py`, `signals/engine.py`, `families.py`, `compute_delta_cvd.py`, `regimes.py` and their tests stay green in place; **no Track B work may modify a Track A module**; shared modules (`backtest`, `horizon`, `base_rates`, `calibration`, `s1`) change additively with Track A's tests as the gate. The whole code change parking requires is one docstring line: `VENUE-SPECIFIC — GC only`. Track A's open threads (Gate 1 option (b), Part C, the filtered exhaustion arm needing ~2.5 years of tape) are recorded as open, not resolved. **Parked on evidence, with the evidence's limits quoted rather than paraphrased** — `FAMILIES.md` §5: *"Does not: say the conditions are wrong in principle — only that these four, at these thresholds, on this half of this month, do not beat their null."* ⚠️ **The tension is named, not smoothed over:** `docs/ARCHITECTURE.md` §1 states the product thesis as order flow — *"that is the whole wedge"* — and Track B does not use it. The information claim still holds (a chart genuinely cannot show CVD), but **an information advantage that does not produce edge is not yet a product**. **Whether the product is still an order-flow product is a product decision, not an architecture one**, it interacts with licensing, and it is logged as open question #1 rather than answered. **Four of nine stages already exist and do not change**, which is the case for this track being cheap; new modules are `instruments.py`, `features/portable.py`, four functions in `strategies.py`, `reach.py`. **Both tracks share the measurement stack deliberately** — it is what makes their results comparable when A restarts. **Rejoin triggers specified now:** Track B finds a conditioning structure and flow is retested *inside* that bucket (most likely and fairest, since `families.py` scored flow against a **pooled** null); Gate 1 option (b); or enough tape for the filtered exhaustion arm. **When flow returns it is a veto layer** — may suppress on GC, never create and never flip — which is what keeps both instruments on one signal set with only filtering different. **Not scheduled**; takes no week from `plans/team/`. **Nothing is built** |
| R4 | 🧵 **Track B split into two lanes — both are moving, and both have shipped** | Varad + Prathamesh, 6 Sep | [`plans/team/strategy-split.md`](team/strategy-split.md). One bucket axis each: Varad owns `atr_bp` and `vol_state` (steps 1, 2, 4, 7), Prathamesh owns `phase` and `instrument` (steps 3, 5, 6). **The seam (step 1) shipped first, both present**, and its point was the fourteen committed signatures — every function in `features/portable.py` and `strategies.py` named and raising `NotImplementedError` on day one, so two people fill in disjoint bodies without either appending to a moving target. ✅ **That claim has now been tested rather than asserted:** both lanes pushed to `main` within minutes on 6 Sep, and the rebase produced **three conflicts — all in files both lanes were expected to touch (the constants block in `portable.py`, §7's table, the daily update) and zero in any function body.** ✅ **All seven §7 pre-commitments are filled** — Varad's three and Prathamesh's four — each in a commit timestamped before the run that reads it. **Steps 2 (M1) and 3 (M3) can now both run**, and the lanes do not meet again until `reach.py` |
| R5 | ⏱️ **Clock lane — step 3 DONE; the clock conditions volatility but produces no edge** | Prathamesh, 6 Sep | Result: [`analysis/M3_CLOCK.md`](../services/signal-data/analysis/M3_CLOCK.md). **All 19 months restored and verified byte-identical against the manifest** ($64.20; 2025-11 failed mid-download with its trades file already complete, so the resume re-billed ~$0.01 not $3.85). **M3 ran on 107,359 legs, both sides, both halves.** **No phase clears cost** — best is `Asia` at +0.572 ticks against a 1.40t round trip, 41% of it; Varad's "no cell clears the cost floor" holds. ⚠️ **And that residual is weaker than it looks, per Varad's `8142625`:** splitting EV into legs that reached a barrier vs legs marked to market at the horizon, **every phase on both sides loses on trades that actually closed** (`ev_barrier` −0.17 to −0.33) and the whole positive contribution is `ev_open` (+0.21 to +0.33) on a *selected* set — a leg that never touched a tight stop is biased toward the target. **The per-phase EV ranking is a residual of two components 20x larger and is not robust**; `Asia` being "best" is not a result. It also reframes Bug 2: the −0.28 I first reported was `ev_barrier`, a real quantity mislabelled as EV. **`London-NY` carries a large, split-surviving reach lift (14 of 16 cells clear MDE) that does not convert**: its `p_target` and `p_stop` rise together, because `atr_bp`'s trailing window sizes the bracket off the quiet hour *before* the 08:30 release. So the phase axis measures **volatility seasonality, not edge** — which is a positive result for the architecture (`reach.py` keys on `phase` for exactly this reason) and a negative one for the strategy. M3 is a bracket-conditioning input for stage 7, not an entry filter. ⚠️ **Four runs were needed and three gave wrong answers — two bugs, both in `m3_profile.py`, both mine.** (a) bucketed in bp while bracketing in fixed ticks, which is ARCHITECTURE §4.6's error inside the file meant to prevent it — it suppressed the power to see the larger effect — `London-NY` went from 3/8 cells clearing MDE to 7/8 — so **I called the wrong phase "the finding" twice**. ⚠️ Note the second-order correction: both phases held their SIGN in all four runs, so **the inversion was in the reading, not the data**, which is worse: nothing in run 1's output looked wrong. Per-run numbers now in `M3_CLOCK.md` §4; (b) priced the 43% of legs that end unresolved at zero P&L, which moved EV from −0.28 ATR to +0.05. Both are the same shape: every component right in isolation, wrong in composition. **The check that catches this class instantly — `horizon.py` says GC is a random walk, so EV must come out ≈0** — is now the first thing to run on any number from this pipeline. ✅ **The tie band came out EMPTY at M3's bracket** (0.001–0.010 ATR; a tie needs one bar spanning 4× ATR, which is 0.13% of bars). ⚠️ **I then over-generalised that and was corrected the same day:** tie frequency is a property of the *bracket*, not the tape — M1's committed grid reaches `0.75×/0.5×`, where **20.45%** of bars can tie, so Varad's `×2` tie-band pass is right and **the tie-resolution argument for `replay.py` stands** for the tight half of the grid. Step 5's budget must not be shrunk on M3's geometry. ⚠️ **The pre-committed kill condition was answered and was the wrong test**: it asks whether the clock moves `p_target` (it does), not whether the movement is worth anything (it is not). **A kill condition here must be stated in EV net of cost.** **Still open — re-checked 17 Sep, and two of these three were stale.** ✅ ~~Varad has not signed the ET-not-UTC boundaries~~ — **SIGNED 2026-09-06**, checkbox and all, in this file's own 6 Sep (night) block: *"ET rather than UTC for the phase boundaries — reviewed and signed off."* **Re-verified empirically today rather than closed by cross-reference:** across all three DST transitions in the archive (2025-03-09, 2025-11-02, 2026-03-08) **zero bars land in the 01:00–03:00 ET window and the only ET hours present are 18:00–23:00** — reproducing the 6 Sep check exactly, because the transitions fall inside the Fri-17:00 → Sun-18:00 break, so `Asia-London` never loses or doubles a day. ✅ ~~`replay.py` unstarted~~ — **DONE, and it is R8**: `replay.py` is 28,945 bytes on disk, 19 months, 89,646 ties reconciled to `reach_table.csv` to the unit. ✅ ~~Genuinely still open: a large directional intraday pattern M3's null cannot control for~~ — **DONE 17 Sep, and R5 is now fully closed.** [`analysis/M3B_DRIFT.md`](../services/signal-data/analysis/M3B_DRIFT.md), pre-committed as `strategy-precommit.md` **§12 in `444881b`, before the first line of code existed**. **Returns, not brackets** (a bracket confounds direction with volatility — the `atr_bp` quiet-hour artefact found independently by `M1_SURFACE.md` §3, `M3_CLOCK.md` §2 and `REACH.md`), and **disjoint windows with an elapsed-time check**, because `resample_bars` drops empty bars so six bars is not always thirty minutes. **Result: 0 of 6 phases clear the cost floor on the residual, and the pattern is retired.** **The core reasoning is the finding: the period's ENTIRE unconditional drift is +0.1878 bp per 30m window against a 0.3504 bp round trip — the whole bull market cannot pay for the trade that would capture it**, so a phase's share of it never had room to clear. ⚠️ **The signs DO replicate** — `London-NY` −0.487 bp and `Asia-London` +0.792 bp, exactly M3's implication, through an instrument with no bracket in it — **but at t = −0.60 and +1.23, with every `edge_lo` below zero, and those two are the THINNEST phases** (814 and 1,223 windows against `Asia`'s 6,508): the two transition phases, the same thinness `REACH.md` found when both lost their CONTRACTING state. **Scored as a retirement, not a hint.** ✅ **Outcome 3 did not fire** — the signs agreeing means the bracket did *not* manufacture M3's asymmetry, so no per-side number goes back into question. ⚠️ **And it corrected a published number: `M3_CLOCK.md` §6's "gold rose 84%" is wrong — the archive rose **55.2%** (2,640.40 → 4,098.60), a figure §12 repeated without checking; correcting it moves the predicted 0.33 bp to 0.241 and *toward* the 0.1878 measured. 6 tests, both guards mutation-verified, 337 passing ✅ **Step 6's step one passes, 7 Sep** — `analysis/SPOT_FEED_CHECK.md`: Dukascopy is reachable, XAUUSD decodes (20,654 quotes/hour, spread median 1.91 bp with real variance p10 1.64 / p90 2.15), and the level cross-checks against our own GC archive at a ~+19 cost-of-carry basis rather than a scale error. **It was never a reachability problem — the host resets a request with no browser `User-Agent`**, which is very likely what `DELTA_CVD_FINDINGS.md` §4 recorded as "unreachable" and which gated this route for weeks. `dukascopy.py` + 7 tests, no network in the suite. ARCHITECTURE §9 open question 3 is answered and §7's doubted precondition (is spot spread degenerate?) is confirmed non-degenerate — the precondition, not the result. **Still open:** no vendor chosen, `XAUUSD` still absent from `instruments.py`, `mid`/`spread_bp`/`quote_rate_z` still signatures, and a bulk pull is ~10,000 hourly files at a 1-in-4 reset rate — a resumable verifying script, not an afternoon |
| R6 | 📊 **Reach table — step 7 DONE; 81% of the grid is answerable and the two axes are correlated** | Varad, 10 Sep | Result: [`analysis/REACH.md`](../services/signal-data/analysis/REACH.md). `reach.py` on all 19 months and both sides — **207,984 legs, 116 minutes, 10,368 rows over 144 cells**, key frozen in `strategy-precommit.md` §10 the day before the file existed. **116 of 144 cells clear the 400-leg floor against a pre-committed prediction of a third to a half** — the table is far fuller than expected, and §5 of `REACH.md` scores the miss rather than quietly moving on. ⚠️ **The finding is that `phase` and `vol_state` are not independent:** both transition phases lose their entire CONTRACTING state (16 of the 28 dead cells), because CONTRACTING is **4.7%** of `London-NY`'s legs and 10.0% of `Asia-London`'s against 28–36% in the three non-handoff phases — a handoff window containing the 08:30 release is a place where volatility is rising by construction. `London-NY` carries 50% more legs than `Asia-London` and loses *more* cells, so size is not the mechanism. **That is `atr_bp`'s quiet-hour-before-the-release artefact for the third time**, after `M1_SURFACE.md` §3 and `M3_CLOCK.md` §2, now from the volatility axis instead of the clock. Tie band reproduces M1 independently (8.2% undecided vs 10.2%). **No edge and none was owed** — `ev_sym` positive on 4.4% of served rows, median −0.0421 ATR, the cost floor; stage 7 tabulates and does not select. **Thresholds are frozen from here** — the surface has been looked at, so a boundary moved now is a fit. Plain-language version with runnable examples for Prathamesh: [`prathamesh/reach-table-explained.md`](team/prathamesh/reach-table-explained.md). **Two things stay open:** the S7 seam mismatches are **CLOSED — S7 frozen 3 of 3 on 10 Sep**: Prathamesh proposed from the render side, Varad and Shreyas both approved verbally that evening (neither signature is a logged artefact from the moment, and the amendment block says so). `contracts.md` now carries S7 as its seventh seam, which it never had — the freeze register did not know the contract existed. Separately, `first_touch` was ~all 116 minutes — a 48× redundancy **hoisted the same night, 116 min → 3.4 min (34×), verified by a cold-cache rebuild reproducing `reach_table.csv` byte for byte**; `first_touch` keeps its signature so there is still one convention. ~~Left open by it: `m1_surface.csv` no longer regenerates exactly~~ — ✅ **CLOSED, and this row was stale for four days.** `cae9b72` regenerated it on 10 Sep and re-derived every number `M1_SURFACE.md` states before the swap; the row was never updated. ✅ **Re-verified empirically 14 Sep rather than taken from the commit message: a full regeneration from current code reproduces the committed file BYTE FOR BYTE** (same md5, `cmp` clean, 3,456 cells / 159 passing / 351 undecided all matching). **That is the stronger result**, because `m1_sweep.py` was modified twice *after* the regeneration — `6358d85` and `c222a2d`'s `GC`→`inst` un-hardcoding — so the surface reproducing today confirms that refactor was numerically neutral **end to end**, where R7 only claimed it for `month_counts`. ⚠️ **The gap it leaves is that nothing guarded it:** `ff569bb` added `ev_null`/`ev_delta` to `surface()` hours after `2abc2b3` committed the CSV, the file sat two columns short for weeks, and it was found by hand rather than by the suite. **`test_the_committed_surface_carries_every_column_surface_writes` now closes that** — the guard `test_reach.py` already puts between the app's fixture and the table, applied to the artifact that actually drifted. Membership not order (order follows the caller's counts frame, not `surface()`); the ~3-minute regeneration proves the *numbers* and is too slow for the suite, this proves the *shape* in milliseconds. **Mutation-verified**, not assumed: adding a column to `surface()` turns it red and names the column. 331 tests |
| R7 | 🌍 **Spot lane — step 6's code is done and tested; the pull is throttled** | Varad, 10 Sep | The cross-instrument check `ARCHITECTURE` §1 calls the strongest validation in this track. **Code path complete:** `m1_sweep` no longer assumes GC (it reached for the instrument in six places — step 1's `TICK` problem one level up), `XAUUSD` is defined with **Dukascopy's price quantum rather than a broker's tick**, which `6358d85`'s tick-free measurement makes safe for geometry and still forbids for cost, and `spot.py` builds bars from the **mid** with the activity column named `ticks` and never `volume`. Written beside `regimes.py` rather than through it — that module is parked Track A and §2 forbids the edit; both it and `s1.minute_bars` hardcode `SESSION_SHIFT` instead of `inst.session_shift`, so **the seam is one field short of complete**. 8 tests, none on the network; `month_counts` reproduces its pre-refactor frames byte for byte; the whole path proven end to end on synthetic bars. ⚠️ **Blocked on data:** Dukascopy throttles by IP and ~40 diagnostic requests tripped it, after which `curl` fails too — so it is neither a client bug nor something the `Accept` fix reaches. One lost hour cost 131s under the first retry policy (~50 min/day, three months unreachable); now fail-fast at ~31s on the reasoning that a cheap repeated pass beats one that grinds. ⛔ **ANSWERED 11 Sep: the block did not clear in 22 hours.** Every request to `datafeed.dukascopy.com` returns a 503, a reset or a timeout — across three header sets, across instruments (`EURUSD` too) and through `curl` as well as `urllib` — while `example.com` and **`www.dukascopy.com` itself** both return 200 in the same second. So it is an **IP-level block on the data endpoint**, not the `User-Agent` problem `SPOT_FEED_CHECK.md` §2 found nor the missing `Accept` header `6358d85` fixed. **`SPOT_FEED_CHECK.md`'s verdict is amended at the top:** "free, complete and decodable" was measured on ONE HOUR and survives; "usable as a backtest source" was never tested and does not — three months is ~2,200 hour-files and 19 months ~13,900, and this endpoint tolerates neither from one address. **No workaround attempted**: pulling from another address changes the question from *is this feed usable* to *is it usable if we route around its limits*, which is the room's call alongside the vendor decision it already owns. **A free feed you cannot bulk-pull is a different proposition from a free feed** · 🔄 **RE-MEASURED 14 Sep, and the block changed shape: it is a budget, not a blanket 503.** Three cold probes returned real payloads (49,523 and 104,222 bytes, plus a legitimate zero-byte Sunday hour), then a pull got **14 of 24 hours on 2026-07-01** before the wall, then **0 of 8 at a 6-second pause** after a 90s cooldown — so roughly **~17 successful requests from cold**, consumed cumulatively, and **a longer pause is not the lever** (0/8 at 6s is worse than 14/24 at 2s). Post-wall failures are immediate rather than timeouts, which is refusal and not congestion. ⚠️ **And the re-measurement exposed a defect that was invisible while everything 503'd: `pull()` resumed by DAY, so a throttled pass discarded every hour it won.** With a budget under 24 the loop is **non-convergent — re-running it forever produces nothing**, which would have read as "the block never cleared". **Fixed: `spot.py` caches each hour as it lands** (`.hours/YYYY-MM-DD/HH.parquet`), the day parquet still written only when all 24 are present so **no day with holes reaches disk**; 2 tests, **330 passing**, mutation verified (disabling the cache lookup turns the new test red), `ruff`/`mypy` clean. **Still open and still Varad's: no spot data is pulled.** ⛔ **AND THE RECOVERY MEASUREMENT ENDS THE ROUTE: 8 attempts across 13h 40m, every one refused** (the probe's own `t+N` labels are wrong — it was written for a 5-minute cadence and the machine slept, so the span is sound and the spacing is not). ~17 requests per budget against a recovery longer than half a day is **~17 hour-files/day: 3 months ≈ 130 days, 19 months ≈ 817**. **Dukascopy is finished as a bulk source from this address** — not "throttled", not "retry tomorrow", and no further fixing of `spot.py` changes it. **The decision is the room's**, and `strategy-reconciliation.md` §415 makes it the last thing between here and `M4 decided` (M1, M3, M2, tick replay, PDE all done): **(1) the MT5 demo account** — §7.4's second free source, named from the start, never tried, free, and it doubles as the platform check open since Day 2; **(2) route around the IP limit** — still refused, on 11 Sep's own reasoning that it changes the question; **(3) reopen the paid `mbp-1` quote pricing**, whose deferral has now expired rather than been resolved. **Recommended: (1)**, and **(1) was chosen the same day** · 🔍 **MT5 DESK-CHECKED 14 Sep BEFORE ANY SIGNUP** — [`SPOT_FEED_CHECK.md`](../services/signal-data/analysis/SPOT_FEED_CHECK.md) §7. ⚠️ **Nothing was run — no account, no terminal, no ticks**; these are sourced documentation claims, not measurements. **Objection one survives:** the official `MetaTrader5` Python package is Windows-only and row B records no Windows or Linux machine in the loop, but the export does not need it — an **MQL5 script runs inside the terminal** (which has a macOS build) and `CopyTicksRange` writes dated CSV, as the `Historex` script already does. So (1) is free **via the script path only**. ⛔ **Objection two resizes the prize: real tick depth is the broker's choice and is commonly 1–3 months, so 19 months should be assumed unavailable until a named broker proves otherwise.** MT5 is a plausible **3-month** source, not a 19-month one — not fatal, since 3 months is the smaller target already in the plan and one dated export satisfies it with no throttle and no resume bookkeeping, but a real downgrade from what the recommendation implied. ⚠️ **And the trap the desk check was worth:** MT5's Strategy Tester documents **generating ticks from an M1 bar with a fixed spread inside the bar** when tick data is missing. Route 2 asks only *does spot spread vary enough to filter on*, so a generated tick **answers the question by construction and answers it wrongly** — a fixed intra-bar spread reads as degenerate, which is the very null under test; such a study would be measuring MetaQuotes' interpolation. Same shape as an absent hour reading as a calm one. ⚠️ **Left open honestly:** generation is documented as *Strategy Tester* behaviour and whether a terminal-side `CopyTicksRange` export can also contain synthesized ticks is **not established** — it needs no ruling, because `MqlTick.flags` settles it per tick. **Acceptance test, and nothing costs a signup until step 3: (i) read the depth in the Symbols panel (`Ctrl+U`) first — a broker that will not serve 3 months of real XAUUSD ticks ends the route before an account exists; (ii) export via `CopyTicksRange` keeping `flags`, never the GUI (the `Flags` column is reported missing from GUI tick CSV, and an unauditable export is worthless here); (iii) a loader must reject a flagless export outright — hard failure, not warning.** **No MT5 loader was written, deliberately** — the contract it must meet is `decode_bi5`'s, and parsing a format nobody here has seen a byte of is the speculative half. **Next action is step (i), five minutes in a terminal, and it is Varad's** · ⛔ **AMENDED after `54ac312`: an MT5 demo account now exists and it is the WRONG ONE.** The gate line #3 demo is a **MetaQuotes demo (XAUUSD M5) in the MT5 *web* terminal**, which looks like step (i) answered and is not. **(a) The web terminal cannot run MQL5 scripts, EAs or indicators** — custom code is desktop-only — so that account supplies **no export path**, and the GUI export it lacks is the flag-dropping one anyway. ⛔ **(b) `MetaQuotes-Demo` is not a broker**: MetaQuotes runs no live venue, the server exists to exercise the platform, its prices are widely reported to diverge from real brokers' (one forum thread asks whether its XAUUSD is artificial, another is titled *"real ticks in MetaQuotes Demo account are missing"*), and MetaQuotes' own guidance is not to rely on the feed. **(b) is disqualifying rather than complicating:** route 2 asks whether **a broker's** spot spread varies enough to filter on, and §6's caveat — *spot spread is a broker pricing decision* — presumes a broker exists. On `MetaQuotes-Demo` none does, so a spread measured there is **MetaQuotes' demo price generator and nothing else** — the same *plausible number from the wrong population* this repo keeps naming, arriving through a convenient account instead of a file copy. ⚠️ **Evidence weighed honestly:** (a) is a documented platform limit; (b) rests on user reports and an absence of any claim to the contrary, which disqualifies the server for a measurement whose whole content is *whose* spread this is, and does **not** support calling the prices fabricated. **Step (i) stands with one name struck out: desktop terminal, real broker's demo server, and `MetaQuotes-Demo` is not a candidate. The existing account closes a gate line; it does not open this route** |
...
| V1 | 🔑 **Vision Hub branch — review, sign S3/S5, merge** | Prathamesh → Varad, Shreyas | `feat/vision-hub`, pushed 18 Sep, nine commits. The backend, portal, site and desktop gate all verified live (Done, 18 Sep). Open: two contract amendments unsigned; `PLACEHOLDER_…` contact/payment/team values; the `.local`-email CLI/validator mismatch; no deployment. **Merging renames the product; that is a room decision, not a PR review.** |
